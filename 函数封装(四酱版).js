...
function next (  ) {
    ...
    for (var i=rowList.length-1;i>=0;i--){
        ...
        for (var j = 0;j<blockList.length;j++){
            ...
            col2.style.backgroundImage=col1.style.backgroundImage;
        }
    }
    ...
    for (var i = 0;i<row1.children.length;i++){
        if (i==newNum){
            row1.children[i].style.backgroundImage = "url(\"四酱.gif\")";
        }else{
            row1.children[i].style.backgroundImage = '';
        }
    }
}
...
function startTime (  ) {
    timerID = setInterval(function (  ) {
        for (var i =0;i<blockList.length;i++){//遍历第四行的所有格子
            if (row4.children[i].style.backgroundImage =="url(\"四酱.gif\")"){//判断第4行的格子有没有加black类,如果有则表示上一次结束前没有清除,
...
//判断按下的键是否为目标键,封装成函数
function isKey ( key ) {
    if (key.style.backgroundImage !="url(\"四酱.gif\")"){//如果按下的键没有black类,则弹出失败并清除定时器
        clearInterval(timerID);
        failed();
...
function level (  ) {
    ...
    speed.innerText = '每 ' + speedLevel/1000 + ' 秒一个四酱';
}

//失败提示封装函数
function failed (  ) {
    clearInterval(timerID);
    fTips.innerText = '本次共消除了' + sum + '个四酱鸭';
