...
function next (  ) {
    ...
    for (var i = 0;i<row1.children.length;i++){
        if (i==newNum){
            row1.children[i].style.backgroundColor = 'rgb('+color1+','+color2+','+color3+')';
        }else{
...
function startTime (  ) {
    timerID = setInterval(function (  ) {
        for (var i =0;i<blockList.length;i++){//遍历第四行的所有格子
            if (row4.children[i].style.backgroundColor!=''){//判断第4行的格子有没有加black类,如果有则表示上一次结束前没有清除,
...
//判断按下的键是否为目标键,封装成函数
function isKey ( key ) {
    if (key.style.backgroundColor==''){//如果按下的键没有black类,则弹出失败并清除定时器
        clearInterval(timerID);
        failed();
